...
class ProfileHandler(BaseHandler):
    ...
    def create_profile(self, token, profile_name_):
        user = self.verify_token(token)
        fs = FileSystemManager()

        profile_object = Profile()
        profile_object.name = profile_name_
        ...
        profile_path = os.path.join(fs.get_dataset_directory_path(),
                                    profile_name_)
        fs.create_dir_if_not_exist(profile_path)
        w_group_dict = work_shift_group.copy()
        w_group_dict[profile] = profile_name_
        ...
        r_group_dict[profile] = profile_name_
