...
class ShiftTypeHandler:
    ...
    def verify_shifts_exist(self, shifts):
        not_exist_shifts = []
        for shift in shifts:
            exist = self.shift_dao.exist(shift)
            if exist is False:
    ...
    def get_by_name(self, token, name):
        verify_token(token, self.user_dao)
        shift_type_dict = self.shift_type_dao.find_by_name(name)
    ...
    def get_all(self, token):
        verify_token(token, self.user_dao)
        return self.shift_type_dao.fetch_all()

    def get_all_names(self, token):
        return [
            shift_type[shift_type_name] for shift_type in self.get_all(token)
        ]
