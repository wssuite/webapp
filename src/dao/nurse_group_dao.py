...
class NurseGroupDao(AbstractDao):
    ...
    def find_by_name(self, name):
        return self.collection.find_one(
            {nurse_group_name: name}, {mongo_id_field: 0}
        )

    def exist(self, name):
        nurse_group = self.find_by_name(name)
    ...
    def get_with_nurses(self, nurses):
        cursor = self.collection.find(
    ...
    def remove(self, name):
        self.collection.find_one_and_delete({nurse_group_name: name})
