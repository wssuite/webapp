...
from test_constants import (
    ...
    early_shift, late_shift, nurse1, nurse_group1, random_hex,
    nurse_group2, min_cons_contract)
...
class TestContractHandler(TestCase):
    def setUp(self) -> None:
        ...
        self.handler.nurse_group_dao.insert_one_if_not_exist(nurse_group1.copy())
        self.handler.nurse_group_dao.insert_one_if_not_exist(nurse_group2.copy())
        self.handler.contract_dao.insert_one(full_time_not_valid_contract_with_general.copy())
    ...
    def insert_missing_deps(self):
        ...
        self.handler.add(random_hex, full_time_valid_contract_with_general.copy())

    def test_contract_addition_if_shift_not_exist_raise_error(self):
        with self.assertRaises(ShiftNotExist):
            self.handler.add(random_hex, full_time_valid_contract_with_general_update_to_invalid)
    ...
    def test_contract_addition(self):
        ...
        self.assertEqual(
            [general_contract_dict,
             full_time_not_valid_contract_with_general,
             min_cons_contract,
             full_time_valid_contract_with_general], all_contracts)

    def test_update_contract_if_update_contradicts_nurse_indirect_contract_raise_error(self):
        self.insert_missing_deps()
    ...
    def test_delete_contract_if_contract_has_depended_object_raise_error(self):
        ...
        with self.assertRaises(CannotDeleteContract):
            self.handler.delete(random_hex,
                                full_time_valid_contract_with_general[
                                    contract_name]
                                )

    def test_delete_contract_if_contract_not_have_depended_object_succeeds(self):
        self.insert_missing_deps()
        self.handler.delete(random_hex,
                            full_time_not_valid_contract_with_general[
                                contract_name]
    ...
    def test_get_all_contract_names(self):
        self.insert_missing_deps()
        expected = [general_contract_dict[contract_name],
